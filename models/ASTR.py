...
class ArgumentAwareMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        weighted_expert_mask = expert_mask * gate_score  # [batch, num_experts]
...
class ASTR(RobertaPreTrainedModel):
    def __init__(self, config, decode_layer_start=17,tokenizer=None):
        ...
        if self.config.architectures[0] == "RobertaForMaskedLM" or self.config.architectures[0] == "ASTR":
            self.roberta = RobertaModel_(config, decode_layer_start=decode_layer_start, structural_mask=config.structural_type)
        else:
            self.roberta = BartModel(config, structural_mask=config.structural_type)
    ...
    def select_template(self, trigger_repr):
        """
        Args:
            trigger_repr: [hidden_size] 触发词的表示
        Returns:
            template_weights: [num_templates] 
            selected_idx: int
        """
        scores = torch.matmul(trigger_repr, self.w_template_select.t())  # [num_templates]
        ...
        for batch_idx, (event_trigger, event_template_options, sample_event_types) in enumerate(zip(event_triggers, template_options, event_types)):           
            ...
            for event_idx, (trigger_pos, event_type) in enumerate(zip(event_trigger, sample_event_types)):
                if event_type in event_type_to_template:
                    ...
                    templates_text_list = event_template_options[event_idx] # list [3]
                    ...
                    all_template_embs = self.roberta.embeddings(
            ...
            while len(batch_dec_prompt_mask_ids) < self.config.max_template_seq_length:
                ...
                for arg_role in selected_arg_joint_prompt[ii].keys():
                    ...
                    for (p_start,p_end, p_start_off, p_end_off) in zip(prompt_slots['tok_s'], prompt_slots['tok_e'], prompt_slots['tok_s_off'], prompt_slots['tok_e_off']):
                        ...
                        if self.config.dataset == 'wikievent':
                            ...
                            prompt_query_sub = context_output[enc_text_prompt_slots:enc_text_prompt_slote]
