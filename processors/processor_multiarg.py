...
class MultiargProcessor(DSET_processor):
    ...
    def set_dec_input(self):
        ...
        if self.args.model_type == "base":
            self.arg_query = True
        elif "ASTR" in self.args.model_type:
    ...
    def convert_examples_to_features(self, examples, role_name_mapping=None):
        ...
        if self.prompt_query:
            prompts = self._read_prompt_group(self.args.ontology_path)
            prompts1 = self._read_prompt_group(self.args.template_path1)
            prompts2 = self._read_prompt_group(self.args.template_path2)
            prompts3 = self._read_prompt_group(self.args.template_path3)
        ...
        for example in examples:
            ...
            for i, event_type in enumerate(event_type_2_events):
                ...
                for event in events:
                    ...
                    template_options = [template_full_1, template_full_2, template_full_3]
                    ...
                    # 每个文本包含多个事件，多个事件循环修改attention_mask的值，直到这个文本的所有事件都处理完毕
